...
flowchart TB
  subgraph ECSF["ECS on Fargate"]
    direction TB
    ECP["コントロールプレーン：無料"]
    ET["Task ＝ Fargate microVM"]
  end
  subgraph EKSF["EKS on Fargate"]
    direction TB
    KCP["コントロールプレーン：$0.10/h"]
    KProf["Fargate Profile<br/>（namespace / label で選択）"]
    KP["1 Pod ＝ 1 Fargate microVM"]
    KProf --> KP
  end
  FG["Fargate 基盤（サーバーレス compute）"]
  ET --> FG
  KP --> FG
  classDef ecs fill:#cde2fb,stroke:#2a78d6,color:#0b0b0b;
  classDef k8s fill:#d3f2e6,stroke:#1baf7a,color:#0b0b0b;
...
flowchart TB
  ...
  subgraph K8SS["Kubernetes：守る層が多い（＝構成ミスの余地が広い）"]
    ...
    KS1["RBAC（過剰権限）"]
    KS2["NetworkPolicy"]
    KS3["PodSecurity / Admission Control"]
    KS4["Secret 管理（平文リスク）"]
    KS5["サプライチェーン / イメージ"]
  end
  CNAPP["CNAPP（CSPM + ランタイム検知<br/>+ アドミッション + イメージスキャン）"]
  KS1 --> CNAPP
  KS2 --> CNAPP
  KS3 --> CNAPP
  KS4 --> CNAPP
